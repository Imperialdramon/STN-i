...
# --show_single_nodes : (Optional) Whether to include SINGLE nodes in the plot.
#                   TRUE or FALSE (default: TRUE).
...
# --shape_option  : (Optional) Integer value (1–5) specifying a shape scheme for node types.
#                   Each option changes how different node types are visually represented (default: 1).
# --size_type    : (Optional) String indicating the size type for nodes. Options:
#                    - "equals"           : All nodes have equal size (default)
#                    - "configurations"   : Size based on number of configurations (on the node)
#                    - "out_degree"       : Size based on out-degree
#                    - "in_degree"        : Size based on in-degree
#                    - "degree"           : Size based on total degree
#                    - "elite_out_degree" : Size based on elite out-degree (to regular nodes)
...
option_list <- list(
  ...
  make_option(c("-n", "--show_single_nodes"),
              type="logical",
              default=TRUE,
              help="Whether to include SINGLE nodes in the plot [default= %default]"),
  ...
  make_option(c("-p", "--palette"),
              type="integer",
              default=1,
              help="Color palette (1-5) for nodes and edges. Palette 1 (default): regular=black, elite=orange, best=red, improving=gray, equal=blue, worsening=green [default= %default]"),

  make_option(c("-w", "--shape_option"),
              type="integer",
              default=1,
              help="Shape scheme (1-3) for node types. Option 1 uses: Start=square, End=triangle, Standard=diamond, Start-End=cross, Start-Standard=star, Standard-End=ellipse, Start-Standard-End=circle [default= %default]"),

  make_option(c("-t", "--size_type"),
              type="character",
              default="equals",
              help="Size type for nodes: equals, configurations, out_degree, in_degree, degree, elite_out_degree [default= %default]"),
...
if (!opt$shape_option %in% 1:3) {
  stop("shape_option must be an integer between 1 and 3")
}

if (!opt$size_type %in% c("equals", "configurations", "out_degree", "in_degree", "degree", "elite_out_degree")) {
  stop("size_type must be one of: equals, configurations, out_degree, in_degree, degree, elite_out_degree")
}
...
STN_i <- stn_i_plot_create(
  ...
  show_single_nodes = opt$show_single_nodes,
  palette_colors = palette_colors,
  shape_option = opt$shape_option,
  size_type = opt$size_type,
