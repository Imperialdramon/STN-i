...
get_stn_i_metrics_elite_nodes <- function(stn_i_result) {
  ...
  metrics$elite_components <- components(ESTN_i)$no
  ...
  if (vcount(ESTN_i) > 0) {
    metrics$max_in_from_elite_degree <- max(degree(ESTN_i, mode = "in"), na.rm = TRUE)
    metrics$min_in_from_elite_degree <- min(degree(ESTN_i, mode = "in"), na.rm = TRUE)
    metrics$average_in_from_elite_degree <- mean(degree(ESTN_i, mode = "in"), na.rm = TRUE)
    metrics$max_out_to_elite_degree <- max(degree(ESTN_i, mode = "out"), na.rm = TRUE)
    metrics$min_out_to_elite_degree <- min(degree(ESTN_i, mode = "out"), na.rm = TRUE)
    metrics$average_out_to_elite_degree <- mean(degree(ESTN_i, mode = "out"), na.rm = TRUE)
    metrics$max_out_to_regular_nodes_degree <- max(V(ESTN_i)$outgoing_regular_nodes, na.rm = TRUE)
    metrics$min_out_to_regular_nodes_degree <- min(V(ESTN_i)$outgoing_regular_nodes, na.rm = TRUE)
    metrics$average_out_to_regular_nodes_degree <- mean(V(ESTN_i)$outgoing_regular_nodes, na.rm = TRUE)
  } else {
    metrics$max_in_from_elite_degree <- NA
    metrics$min_in_from_elite_degree <- NA
    metrics$average_in_from_elite_degree <- NA
    metrics$max_out_to_elite_degree <- NA
    metrics$min_out_to_elite_degree <- NA
    metrics$average_out_to_elite_degree <- NA
    metrics$max_out_to_regular_nodes_degree <- NA
    metrics$min_out_to_regular_nodes_degree <- NA
    metrics$average_out_to_regular_nodes_degree <- NA
  }
